...
if not os.path.exists("maryam_screenshots(8)"):
    os.makedirs("maryam_screenshots(8)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"maryam_screenshots(8)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
        ...
        if time.time() - start_time >= 30:
            ...
            if not df.empty:
                df.to_excel('maryam_attendance_with_attention_and_average(8).xlsx', index=False)
                print("Attendance saved to 'maryam_attendance_with_attention_and_average(8).xlsx'.")
    ...
    if not df.empty:
        ...
        df.to_excel('maryam_attendance_with_attention_and_average(8).xlsx', index=False)
        print("Attendance with attentiveness and average score saved to 'maryam_attendance_with_attention_and_average(8).xlsx'.")
